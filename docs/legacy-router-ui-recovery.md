...
verified. Replace `RESCUE_SHA256_AFTER_FINAL_BUILD` and
`IMMUTABLE_SOURCE_COMMIT_AFTER_MERGE` only from the final staged evidence.
...
echo 'RESCUE_SHA256_AFTER_FINAL_BUILD  rescue-router-ui.sh' | sha256sum -c -
...
`IMMUTABLE_SOURCE_COMMIT_AFTER_MERGE` and as the exact 0.7.11 release asset.
Its SHA-256 is `RESCUE_SHA256_AFTER_FINAL_BUILD`. It accepts only the versions
