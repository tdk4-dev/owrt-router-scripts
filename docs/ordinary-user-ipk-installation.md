# Installing Router UI 0.7.11 RC12 on an existing RD23

> **Draft RC12 guidance — not an installation authorization.** Do not use this
...
Router UI RC12 is prerelease software. LuCI displays `0.7.11-rc.12` and a
**Release candidate** warning. The OpenWrt packages use
`0.7.11~rc12-1`; that ordering lets the signed updater offer the later stable
...
ssh root@ROUTER_ADDRESS \
  'sysupgrade -b /tmp/router-before-0.7.11-rc12.tar.gz'
ssh root@ROUTER_ADDRESS \
  'cat /tmp/router-before-0.7.11-rc12.tar.gz' \
  > router-before-0.7.11-rc12.tar.gz
test -s router-before-0.7.11-rc12.tar.gz
tar -tzf router-before-0.7.11-rc12.tar.gz >/dev/null
router_backup_sha="$(ssh root@ROUTER_ADDRESS \
  "sha256sum /tmp/router-before-0.7.11-rc12.tar.gz | awk '{print \$1}'")"
local_backup_sha="$(shasum -a 256 router-before-0.7.11-rc12.tar.gz | awk '{print $1}')"
...
## 2. Copy the signed RC12 package set and first-install bootstrap
...
SHA256SUMS
SHA256SUMS.sig
premier-router-core_0.7.11~rc12-1_all.ipk
luci-app-premier-router_0.7.11~rc12-1_all.ipk
premier-router-setup_0.7.11~rc12-1_all.ipk
...
remaining storage no longer meets the RC12 preflight requirement. Do not use
...
subset of the exact RC12 packages, rerunning the same verified bootstrap and
...
- all three package versions: `0.7.11~rc12-1`;
- displayed Router UI version: `0.7.11-rc.12`;
...
`0.7.11` metadata is published, the page will recognize it as newer than RC12.
