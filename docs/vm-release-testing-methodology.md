...
- QEMU is diagnostic-only for RC13 and cannot authorize candidate or release
...
boot ID, and cover RC5 → RC13 candidate preview, post-reboot commit, adoption,
...
localhost access. `validate-rc13-virtualbox-evidence.sh` enforces this contract.
...
`.github/workflows/publish-router-ui-rc13-virtualbox-evidence.yml`. Do not merge
...
- a root-owned, read-only staged tree at
  `/srv/router-ui-evidence/rc13-final/<source-sha>` and the matching signed
  release at
  `/srv/router-ui-evidence/releases/<source-sha>/release-v0.7.11-rc.13`;
...
This section describes later product-image qualification, not the RC13 Phase 1
...
as acceptance criteria for RC13. Phase 1 tests only the retained VPN,
