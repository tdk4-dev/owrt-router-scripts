...
- QEMU is diagnostic-only for RC10 and cannot authorize candidate or release
...
boot ID, and cover RC5 → RC10 candidate preview, post-reboot commit, adoption,
...
localhost access. `validate-rc10-virtualbox-evidence.sh` enforces this contract.
...
`.github/workflows/publish-router-ui-rc10-virtualbox-evidence.yml`. Do not merge
...
- a root-owned, read-only staged tree at
  `/srv/router-ui-evidence/rc10-final/<source-sha>` and the matching signed
  release at
  `/srv/router-ui-evidence/releases/<source-sha>/release-v0.7.11-rc.10`;
...
This section describes later product-image qualification, not the RC10 Phase 1
...
as acceptance criteria for RC10. Phase 1 tests only the retained VPN,
