...
- QEMU is diagnostic-only for RC8 and cannot authorize candidate or release
...
boot ID, and cover RC5 → RC8 candidate preview, post-reboot commit, adoption,
...
localhost access. `validate-rc8-virtualbox-evidence.sh` enforces this contract.
...
`.github/workflows/publish-router-ui-rc8-virtualbox-evidence.yml`. Do not merge
...
- a root-owned, read-only staged tree at
  `/srv/router-ui-evidence/rc8-final/<source-sha>` and the matching signed
  release at
  `/srv/router-ui-evidence/releases/<source-sha>/release-v0.7.11-rc.8`;
...
This section describes later product-image qualification, not the RC8 Phase 1
...
as acceptance criteria for RC8. Phase 1 tests only the retained VPN,
