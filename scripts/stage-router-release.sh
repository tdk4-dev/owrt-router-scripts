...
pr_version_valid "$APP_VERSION" || fail "application version is malformed"
...
if [ "$APP_VERSION" = 0.7.11 ]; then
  ...
  # Every post-bridge release requires the package-managed v2 source. Legacy
  # routers must install the immutable 0.7.11 bridge through generic rescue.
  TRANSITIONS_JSON='[{"source_version":"0.7.11","source_protocol":2,"mode":"package-v2"}]'
  COMPAT_TRANSPORT_JSON='null'
fi
...
jq -n \
  ...
  --argjson compatibility_transport "$COMPAT_TRANSPORT_JSON" \
...
if [ "$APP_VERSION" = 0.7.11 ]; then
  ...
  (cd "$RELEASE_DIR" && sha256sum luci-vpn-ui.tar.gz > luci-vpn-ui.tar.gz.sha256)
fi

for file in install-router-ui-release.sh rescue-router-ui.sh; do
