...
const OUTPUT = outputArgument >= 0
  ...
  : resolve(ROOT, 'docs/evidence/router-ui-0.7.11-rc8-scope-ledger.json');
...
function currentMainEffect(filePath) {
  const effects = {
    'README.md': 'Preserve current-main product, registry, ownership, and release-state documentation; apply only truthful RC8 Phase 1 deltas.',
    ...
    'tests/test-router-metadata.sh': 'Preserve ownership metadata behavior with RC8 identity and without restoring deferred owner-preparation UI.',
...
function effect(filePath, cls) {
  ...
  return 'Implements or verifies the 0.7.11 trust/update bridge, package boundary, release-safety contract, updater recovery, or diagnostic infrastructure.';
}
...
const entries = changed.map((line) => {
  ...
  return {
    ...
    rc8_decision: decision(filePath, cls),
    review_status: 'reviewed-no-unexplained-scope'
  };
});
...
if (entries.some((entry) => !entry.user_visible_or_operational_effect || !entry.rc8_decision))
...
const ledger = {
  ...
  summary: {
    ...
    decision_counts: Object.fromEntries(['preserve', 'exclude', 'reimplement'].map((name) => [
      name, entries.filter((entry) => entry.rc8_decision === name).length
    ]))
  },
  audit_rules: {
