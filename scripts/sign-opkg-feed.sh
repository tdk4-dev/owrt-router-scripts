...
fail() { printf 'ERROR: %s\n' "$*" >&2; exit 1; }
[ "$APP_VERSION" = 0.7.11 ] || fail "feed signing is scoped to 0.7.11"
...
jq -n --arg version "$APP_VERSION" --arg package_version "$PKG_VERSION" \
  ...
  --arg signing_key_fingerprint "$RELEASE_KEY_FINGERPRINT" --argjson packages "$packages" \
  '{schema_version:1,kind:"opkg-feed",channel:"candidate",app_version:$version,
...
printf 'Signed candidate opkg feed with key %s (%s)\n' "$RELEASE_KEY_ID" "$RELEASE_KEY_FINGERPRINT"
