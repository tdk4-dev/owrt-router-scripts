...
if [ "$RELEASE_CHANNEL" = stable ]; then
  ...
  jq -e '
    .app_version == "0.7.11-rc.17" and .package_version == "0.7.11~rc17-1" and
    any(.transitions[];
      .source_version == "0.7.11-rc.5" and .source_protocol == 2 and
      .mode == "package-v2-rc") and
    any(.transitions[];
    ...
    any(.transitions[];
      .source_version == "0.7.11-rc.15" and .source_protocol == 2 and
      .mode == "package-v2-rc")
  ' "$MANIFEST" >/dev/null || fail "RC17 release does not authorize the required protocol-2 transitions"
  jq -e 'any(.transitions[]; .source_version == "0.7.11-rc.16" and
    .source_protocol == 2 and .mode == "package-v2-rc")' "$MANIFEST" >/dev/null ||
    fail "RC17 release does not authorize the RC16 protocol-2 transition"
fi
