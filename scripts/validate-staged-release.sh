...
if [ "$RELEASE_CHANNEL" = stable ]; then
  jq -e '
    ...
    any(.transitions[];
      ...
      .mode == "package-v2-rc")
  ' "$MANIFEST" >/dev/null || fail "stable release does not authorize the RC15 protocol-2 transition"
else
  jq -e '
    ...
    any(.transitions[];
      ...
      .mode == "package-v2-rc")
  ' "$MANIFEST" >/dev/null || fail "RC15 release does not authorize the RC5, RC6, and invalidated RC7 protocol-2 transitions"
fi
