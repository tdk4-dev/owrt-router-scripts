Router UI 0.7.11 RC13

Router UI `0.7.11-rc.13` is the conservative continuation candidate identity for the
0.7.11 trust/update bridge. Its OpenWrt package version is `0.7.11~rc13-1`, its
channel is `candidate`, its future tag convention is
`vpn-panel-v0.7.11-rc.13`, and its stable successor is `0.7.11` /
...
RC13 keeps the narrow legacy-rescue repair: after authenticating the target manifest
...
RC13 also treats that metadata creation as one explicit first-install
...
RC13 also makes the global VPN control persistent for the standard OpenWrt
...
RC13 is not immutable release evidence merely because this identity is present
...
RC13 preserves protocol-2 signed-manifest verification, transactional package
...
RC13. The missing signed-manifest digest for the first RC6 byte set remains
