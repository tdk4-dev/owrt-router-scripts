...
pr_version_newer() {
  ...
  awk -v left="$1" -v right="$2" 'BEGIN {
    lrc = rrc = -1
    if (match(left, /RC[0-9]+$/)) {
      lrc = substr(left, RSTART + 2) + 0
      left = substr(left, 1, RSTART - 1)
    }
    if (match(right, /RC[0-9]+$/)) {
      rrc = substr(right, RSTART + 2) + 0
    ...
    if (lrc < 0 && rrc >= 0) exit 0
    if (lrc >= 0 && rrc < 0) exit 1
    if (lrc > rrc) exit 0
    exit 1
  }'
}
...
pr_manifest_validate() {
  ...
  for object in transaction_supervisor update_library compatibility.status_0_7_9 standalone_installer rescue; do
    ...
    case "$object" in
      ...
      update_library|compatibility.status_0_7_9|rescue) [ "$object_protocol" = 1 ] ;;
