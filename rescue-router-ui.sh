...
case "$SOURCE_VERSION" in
  ...
  0.7.11)
    if [ -x /usr/sbin/vpn-ui-update ] &&
      ...
      printf 'Router UI 0.7.11 and updater protocol v2 are already installed.\n'
      exit 0
    fi
    die "0.7.11 metadata exists but protocol-v2 validation failed"
    ;;
  *RC*|*dev*|*dirty*|development|'')
...
fetch "$RELEASE_BASE/router-release-manifest.json" "$MANIFEST" ||
  die "could not download the exact 0.7.11 manifest"
fetch "$RELEASE_BASE/router-release-manifest.json.sig" "$SIGNATURE" ||
  die "could not download the exact 0.7.11 manifest signature"
usign -q -V -p "$PUBLIC_KEY" -m "$MANIFEST" -x "$SIGNATURE" ||
  die "0.7.11 manifest signature verification failed"
...
[ "$(sed -n '1p' "$VERSION_FILE" | tr -d '\r\n')" = "$TARGET_VERSION" ] ||
  die "installed app version is not 0.7.11"
...
for package in premier-router-core luci-app-premier-router premier-router-setup; do
  ...
  [ "$version" = 0.7.11-1 ] || die "$package is not installed at 0.7.11-1"
done
...
printf 'Router UI 0.7.11 is installed with updater protocol v2.\n'
