...
bash -n "$CONTROLLER"
grep -Fq 'SSL_CERT_FILE=/etc/ssl/certs/router-ui-vm-ca.pem' "$CONTROLLER"
grep -Fq '/usr/sbin/vpn-ui-update check-start' "$CONTROLLER"
grep -Fq '/usr/sbin/vpn-ui-update apply-start' "$CONTROLLER"
grep -Fq 'detached stable update apply did not reach pending commit' "$CONTROLLER"
! grep -Fq 'VPN_UI_SYNC_WORKER=1' "$CONTROLLER"
