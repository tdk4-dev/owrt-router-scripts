...
grep -q 'Local harness HEAD differs from the candidate source SHA' "$WRAPPER"
grep -q 'staged Mac Pro harness differs from the attributed candidate source' "$CONTROLLER"
grep -q 'staged HTTPS harness differs from the attributed candidate source' "$CONTROLLER"
grep -q 'staged guest harness differs from the attributed candidate source' "$CONTROLLER"
