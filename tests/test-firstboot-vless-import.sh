...
case "$1" in
  ...
  add)
    ...
    ;;
  subscription-add)
    printf '%s\n' '{"ok":true,"selected":"subscription-profile-id","profiles":[{"id":"subscription-profile-id"}]}'
    ;;
  *)
...
import_subscription_profile 'https://provider.example/sub/redacted'
[ "$VPN_PROFILE_ID" = "subscription-profile-id" ]

printf '%s\n' "first-boot VPN import ID regression test passed"
