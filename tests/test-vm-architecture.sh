...
VIRTUALBOX_PUBLISH="$ROOT_DIR/.github/workflows/publish-router-ui-rc10-virtualbox-evidence.yml"
VIRTUALBOX_VALIDATOR="$ROOT_DIR/tests/integration/validate-rc10-virtualbox-evidence.sh"
...
grep -q 'publish-router-ui-rc10-virtualbox-evidence.yml' "$CANDIDATE" ||
...
grep -q 'validate-rc10-virtualbox-evidence.sh' "$CANDIDATE" ||
...
grep -q 'provenance_member=' "$CANDIDATE" &&
  ...
  fail 'x86 does not derive its writable extent from exact RD23 stock provenance'
...
! sed -n '/normal_reboot()/,/^}/p' "$GATE" | grep '^  wait_ssh$' >/dev/null ||
