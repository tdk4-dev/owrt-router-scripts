...
jq -n --arg source "$SOURCE" --arg key "$KEY_ID" --arg fingerprint "$FINGERPRINT" '
  {source_commit:$source,source_dirty:false,channel:"candidate",
   app_version:"0.7.11-rc.6",package_version:"0.7.11~rc6-1",
   release_tag:"vpn-panel-v0.7.11-rc.6",signing_key_id:$key,
...
for index in $(seq 1 23); do
  ...
  jq -n --arg source "$SOURCE" --arg baseline "$BASELINE" --arg fingerprint "$FINGERPRINT" '
    {candidate_source_sha:$source,production_public_key_fingerprint:$fingerprint,
     ...
     candidate:{app_version:"0.7.11-rc.6",package_version:"0.7.11~rc6-1",
       release_tag:"vpn-panel-v0.7.11-rc.6"},
  ...
  jq -n --arg source "$SOURCE" --arg key "$KEY_ID" --arg fingerprint "$FINGERPRINT" \
    ...
    {schema_version:1,source_commit:$source,contract_mode:"rc6-active-key",signing_key_id:$key,
     signing_key_fingerprint:$fingerprint,
     candidate:{app_version:"0.7.11-rc.6",package_version:"0.7.11~rc6-1",
       release_tag:"vpn-panel-v0.7.11-rc.6",manifest_sha256:$candidate_hash},
     successor:{app_version:"0.7.11",package_version:"0.7.11-1",
...
for version in 0.7.9 0.7.10; do
  jq -nc --arg name "$version->0.7.11-rc.6->rollback" '
...
jq -nc '{kind:"image",name:"clean-x86-boot",status:"pass",details:{}}' \
  >> "$case_one/transition-results.jsonl"
jq -nc '{kind:"protocol-v2",name:"0.7.11-rc.6->0.7.11->0.7.11-rc.6->0.7.11->0.7.11-rc.6",
...
for phase in before-reboot after-reboot; do
  for sample in $(seq 1 15); do
    jq -nc --arg phase "$phase" --arg boot "boot-$phase" '
      {phase:$phase,boot_id:$boot,app_version:"0.7.11-rc.6",
       package_version:"0.7.11~rc6-1",mem_total_kib:235968,mem_available_kib:81920,
...
jq -e --arg source "$SOURCE" '
  ...
  .rc5_rc6_virtualbox_cycle == "pending-mac-pro" and
