...
for package in premier-router-core luci-app-premier-router premier-router-setup; do
  printf '%s\n' "$package" > "$TMP_ROOT/release/${package}_${PKG_VERSION}_all.ipk"
done
IMAGE="premier-router-$APP_VERSION-openwrt-24.10.5-x86-64.tar.gz"
printf 'x86 validation image\n' > "$TMP_ROOT/release/$IMAGE"
packages="$(
  for package in premier-router-core luci-app-premier-router premier-router-setup; do
    file="$TMP_ROOT/release/${package}_${PKG_VERSION}_all.ipk"
...
jq -n --arg source_commit "$SOURCE_COMMIT" --arg key "$KEY_ID" \
  --arg app_version "$APP_VERSION" --arg package_version "$PKG_VERSION" \
...
FACTORY_PRODUCT_VERSION="$APP_VERSION" \
...
jq -e --arg commit "$SOURCE_COMMIT" --arg app "$APP_VERSION" --arg package "$PKG_VERSION" '
  .schema_version == 2 and .semantic_version == $app and
  .product_version == $app and .package_release == $package and
  ...
  .artifacts[0].variant == "virtualbox" and
  .required_tmp_free_bytes == 1131520
' "$TMP_ROOT/release/factory-release-contract.json" >/dev/null
jq -e --arg app "$APP_VERSION" --arg package "$PKG_VERSION" '
  .schema_version == 2 and .app_version == $app and
  .package_version == $package and .channel == "candidate" and
