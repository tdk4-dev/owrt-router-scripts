...
[ "$VERSION" = "0.7.11-rc.17" ]
[ "$PACKAGE_VERSION" = "0.7.11~rc17-1" ]
[ "$INSTALLED_VERSION" = "$VERSION" ]

. "$UPDATE_LIB"
pr_version_newer 0.7.11-rc.17 0.7.11-rc.16
! pr_version_newer 0.7.11-rc.17 0.7.11
pr_package_version_matches_app 0.7.11-rc.17 0.7.11~rc17-1
...
! pr_version_newer 0.7.11-rc.6 0.7.11-rc.16
...
printf '%s\n' "$STATUS_JSON" | jq -e '
  .current == "0.7.11-rc.17" and .current_channel == "candidate" and
  .latest == "0.7.11" and .available == true
' >/dev/null

jq -e '
  .target == "0.7.11-rc.17" and .package_version == "0.7.11~rc17-1" and
  ...
  any(.baselines[]; .version == "0.7.11-rc.11" and
    ...
    .evidence == "invalidated-real-browser-xray-toggle") and
  any(.baselines[]; .version == "0.7.11-rc.13" and
    .tag_commit == "2a5fcb2a37a6697f8c83a48610ce456a158d8cab" and
    .tree == "9031cebf3f7dd7d02a12692011a28cdf9162a006" and
    .support == "unsupported" and
    .evidence == "rejected-browser-async-state") and
  any(.baselines[]; .version == "0.7.11-rc.14" and
    .tag_commit == "7fb0ecb35c54e34e1c8eedf47bbe0848cf3e9866" and
