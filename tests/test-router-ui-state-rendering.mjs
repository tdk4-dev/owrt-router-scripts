...
for (const permission of [true, false]) {
	...
	const { page } = makePage('update', permission, async (command, args) => {
		...
		return { stdout: JSON.stringify({ ok: true, current: '0.7.11-rc.14' }) };
	});
	const data = {
		current: '0.7.11-rc.14', current_channel: 'candidate', latest: '0.7.11',
	...
	if (permission) {
		...
		assert.equal(status.current, '0.7.11-rc.14');
