...
pr_version_newer 0.7.11-rc.12 0.7.11-rc.7
...
! pr_version_newer 0.7.11-rc.7 0.7.11-rc.12
...
[ "$(pr_offer_identity_decision 0.7.11-rc.7 0.7.11-rc.12 '' '')" = upgrade ]
[ "$(pr_offer_identity_decision 0.7.11-rc.8 0.7.11-rc.12 '' '')" = upgrade ]
[ "$(pr_offer_identity_decision 0.7.11-rc.10 0.7.11-rc.12 '' '')" = upgrade ]
[ "$(pr_offer_identity_decision 0.7.11-rc.11 0.7.11-rc.12 '' '')" = upgrade ]
[ "$(pr_offer_identity_decision 0.7.11-rc.12 0.7.11 "$HASH_A" "$HASH_B")" = upgrade ]
[ "$(pr_offer_identity_decision 0.7.11-rc.12 0.7.11-rc.12 "$HASH_A" "$HASH_A")" = repeat-identical ]
if pr_offer_identity_decision 0.7.11-rc.12 0.7.11-rc.12 "$HASH_A" "$HASH_B" >/dev/null; then
  printf 'different bytes were accepted under the consumed RC12 identity\n' >&2
  exit 1
fi
if pr_offer_identity_decision 0.7.11 0.7.11-rc.12 '' '' >/dev/null; then
  printf 'stable-to-RC12 downgrade was accepted\n' >&2
  exit 1
fi
...
J_SOURCE_TYPE=ipk
J_TARGET_APP_VERSION=0.7.11-rc.12
J_TARGET_PACKAGE_VERSION=0.7.11~rc12-1
J_TARGET_TAG=vpn-panel-v0.7.11-rc.12
