...
VERSION=0.8.0RC2
PACKAGE_VERSION=0.8.0RC2-1
SOURCE_COMMIT=1234567890abcdef1234567890abcdef12345678
SOURCE_DIRTY=0
BUILD_DATE=
EOF
cat > "$TMP_DIR/manifest.json" <<'EOF'
{"package_version":"0.8.0RC2-1","generated_at":"2026-07-11T00:00:00Z"}
EOF
...
printf '%s\n' "$defaults" | grep -q '"version":"0.8.0RC2"'
...
printf '%s\n' "$defaults" | grep -q '"footer_label":"Router Scripts v0.8.0RC2 (manual-ipk-install, support: self-managed, registration: local-only, support access: disabled)"'
...
printf '%s\n' "$build" | grep -q '"package_version":"0.8.0RC2-1"'
