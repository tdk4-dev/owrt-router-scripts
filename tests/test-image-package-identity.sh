...
QEMU_PROCESS_AWK='$1 ~ /(^|\/)qemu-system-x86_64$/ && $0 ~ /(^|[[:space:]])-name[[:space:]]router-ui-vm-/ { count++ } END { print count + 0 }'
grep -q 'ps -eo args=' "$QEMU_GUARD"
grep -q 'ps -eo args=' "$VM_GATE"
[ "$(printf '%s\n' 'qemu-system-x86_64 -name router-ui-vm-bare -m 256' \
  '/usr/bin/qemu-system-x86_64 -m 256 -name router-ui-vm-path' |
  awk "$QEMU_PROCESS_AWK")" -eq 2 ]
[ "$(printf '%s\n' '/bin/bash qemu-guard.sh qemu-system-x86_64 -name router-ui-vm-wrapper' |
  awk "$QEMU_PROCESS_AWK")" -eq 0 ]
