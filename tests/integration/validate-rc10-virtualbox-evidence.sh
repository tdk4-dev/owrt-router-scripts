...
jq -e \
  ...
  .transition.target.app_version == "0.7.11-rc.10" and
  .transition.target.package_version == "0.7.11~rc10-1" and
...
jq -e --arg source "$EXPECTED_SOURCE_SHA" --arg manifest "$release_manifest_sha" '
  ...
  (.clone_name | startswith("RouterUI-RC10-Valera-")) and
...
jq -e '
  ...
  .target.app_version == "0.7.11-rc.10" and
  .target.package_version == "0.7.11~rc10-1" and .target.protocol == 2 and
...
jq -e '
  ...
  .update_page_rc_badge == "0.7.11-rc.10" and
...
printf 'Mac Pro VirtualBox-only RC10 qualification evidence passed for %s\n' \
  "$EXPECTED_SOURCE_SHA"
