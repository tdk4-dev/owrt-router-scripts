...
VALIDATOR="$ROOT_DIR/tests/integration/validate-rc9-virtualbox-evidence.sh"
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/rc9-virtualbox-evidence.XXXXXX")"
...
printf 'name="RouterUI-RC9-Valera-test"\nVMState="poweroff"\n' \
...
jq -n --arg source "$SOURCE_SHA" --arg manifest "$manifest_sha" '{
  ...
  clone_name:"RouterUI-RC9-Valera-test",clone_uuid:"22222222-2222-4222-8222-222222222222",
...
jq -n '{schema_version:1,
  ...
  target:{app_version:"0.7.11-rc.9",package_version:"0.7.11~rc9-1",protocol:2},
  ...
  config_sha256_after_adoption:("a"*64),exact_rc5_rollback:true,rc9_reapplied:true,
  packages:[
    {name:"premier-router-core",version:"0.7.11~rc9-1",sha256:("b"*64)},
    {name:"luci-app-premier-router",version:"0.7.11~rc9-1",sha256:("c"*64)},
    {name:"premier-router-setup",version:"0.7.11~rc9-1",sha256:("d"*64)}]
...
jq -n '{schema_version:1,host_os:"Darwin",same_workstation:true,browser:"Firefox",
  ...
  update_page_rc_badge:"0.7.11-rc.9",vpn_page_mode:"adopted-overlay",console_errors:0
