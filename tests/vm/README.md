...
3. `diagnose-router-ui-vm.yml` is manually dispatched with only the candidate
   ...
   permission. Its QEMU results never authorize an RC15 candidate or release.
4. `publish-router-ui-rc15-virtualbox-evidence.yml` runs from `main` only on a
...
manually dispatched. For pre-merge RC15 qualification, merge a separate
workflow-only bootstrap change containing exactly
`.github/workflows/publish-router-ui-rc15-virtualbox-evidence.yml`; never merge
RC15 merely to expose the workflow. Dispatch the publisher on `main`, record
...
identities to the RC15 protected-candidate workflow.
...
artifact. These diagnostics are not RC15 release evidence.
...
Baseline and QEMU diagnostic artifacts are not release authorization. RC15
...
bundle that passes `validate-rc15-virtualbox-evidence.sh`.
