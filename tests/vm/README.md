...
3. `diagnose-router-ui-vm.yml` is manually dispatched with only the candidate
   ...
   permission. Its QEMU results never authorize an RC14 candidate or release.
4. `publish-router-ui-rc14-virtualbox-evidence.yml` runs from `main` only on a
...
manually dispatched. For pre-merge RC14 qualification, merge a separate
workflow-only bootstrap change containing exactly
`.github/workflows/publish-router-ui-rc14-virtualbox-evidence.yml`; never merge
RC14 merely to expose the workflow. Dispatch the publisher on `main`, record
...
identities to the RC14 protected-candidate workflow.
...
artifact. These diagnostics are not RC14 release evidence.
...
Baseline and QEMU diagnostic artifacts are not release authorization. RC14
...
bundle that passes `validate-rc14-virtualbox-evidence.sh`.
