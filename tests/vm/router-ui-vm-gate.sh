...
load_release_contracts() {
  ...
  if [[ "$ALLOW_LEGACY_CONTRACT" = 1 ]]; then
    ...
    [[ "$CANDIDATE_APP_VERSION" = 0.7.11-rc.13 ]] ||
      fail "candidate app version is not the RC13 contract: $CANDIDATE_APP_VERSION"
    [[ "$CANDIDATE_PACKAGE_VERSION" = '0.7.11~rc13-1' ]] ||
      fail "candidate package version is not the opkg-safe RC13 contract: $CANDIDATE_PACKAGE_VERSION"
