...
jq -e '
  ...
  .release_tag == "vpn-panel-v0.7.11-rc.9" and .channel == "candidate" and
  .source_dirty == false
' "$release_manifest" >/dev/null || fail "immutable transition candidate is not the exact RC9 contract"
