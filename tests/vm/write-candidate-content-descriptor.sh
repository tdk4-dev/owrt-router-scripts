...
key_fingerprint="$(sed -n '1p' "$ROOT_DIR/release/keys/router-ui-production.fingerprint")"
jq -e --arg source "$source_sha" --arg key "$key_id" \
  '.source_commit == $source and .source_dirty == false and .app_version == "0.7.11" and
   .signing_key_id == $key' "$RELEASE_DIR/router-release-manifest.json" >/dev/null
jq -e --arg source "$source_sha" --arg key "$key_id" \
  '.source_commit == $source and .source_dirty == false and .app_version == "0.7.12" and
   .signing_key_id == $key' "$SYNTHETIC_DIR/router-release-manifest.json" >/dev/null
